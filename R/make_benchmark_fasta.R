...
#' Write a synthetic FASTQ file for benchmarking
#'
#' Creates a FASTQ file where each record has a deterministic ID
#' (`seq00001`, `seq00002`, ...) and randomly sampled DNA sequence/quality
#' strings of fixed width.
#'
#' @param path Output file path. If it ends in `.gz` (case-insensitive), output
#'   is gzip-compressed; otherwise, plain text FASTQ is written.
#' @param n Number of FASTQ records to write.
#' @param width Sequence width (bases) per record.
#' @param alphabet Character vector of symbols used to generate sequence lines.
#' @param qual_alphabet Character vector of symbols used to generate quality
#'   lines.
#'
#' @return Invisibly returns `path`.
#'
...
#' make_benchmark_fastq(path, n = 3, width = 10)
#' file.exists(path)
#' unlink(path)
#'
#' @export
make_benchmark_fastq <- function(
  path,
  n = 5000,
  width = 80,
  alphabet = c("A", "C", "G", "T"),
  ...
  qual_alphabet <- as.character(qual_alphabet)
  if (
    length(qual_alphabet) < 1L ||
      anyNA(qual_alphabet) ||
      any(!nzchar(qual_alphabet))
  ) {
    stop(
      "`qual_alphabet` must contain at least one non-empty symbol.",
      call. = FALSE
    )
  }
  ...
  for (i in seq_len(n)) {
    seq <- paste0(sample(alphabet, width, replace = TRUE), collapse = "")
    qual <- paste0(sample(qual_alphabet, width, replace = TRUE), collapse = "")
    writeLines(c(sprintf("@seq%05d", i), seq, "+", qual), con = con)
  }
